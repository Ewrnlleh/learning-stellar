...
impl AtomicSwapContract {
    ...
    pub fn swap(
        ...
        // Require authorization for a subset of arguments specific to each party.
        // Notice, that arguments are symmetric.
        // There is no difference between a and b in the call and hence their signatures can be used either for 'a' or for 'b' role.
...
fn move_token(
    ...
    // the signature from 'to' address (so that parties don't need to know each other).
    token_client.transfer(from, &contract_address, &max_spend_amount);

    // Transfer the necessary amount to 'to'.
    token_client.transfer(&contract_address, to, &transfer_amount);

    // Refund the remaining balance to 'from'.
    if max_spend_amount > transfer_amount {
        let refund = max_spend_amount - transfer_amount;
        token_client.transfer(&contract_address, from, &refund);
    }
}